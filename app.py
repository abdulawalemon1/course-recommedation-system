...
def get_recommendation(title, cosine_sim_mat, df, num_of_rec=5):
    ...
    final_recommended_coures = result_df[[
        'course_title', 'similarity_score', 'url', 'price', 'num_subscribers']]
    return final_recommended_coures.head(num_of_rec)
...
@st.cache
def search_item_if_not_found(term, df):
        ...
        if st.button("Recommend"):
            if search_term is not None:
                try:
                    ...
                    result_df = search_item_if_not_found(search_term, df)
